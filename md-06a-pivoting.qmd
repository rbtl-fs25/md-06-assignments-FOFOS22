...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    It's kind of a toolbox that gives you a variety of options to restructure and reorganize your data.
...
```{r}
waste_data_longer <- waste_data_fct |> 
  pivot_longer(
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report? nope
...
ggplot(data = waste_data_long_mean,
  ...
  labs(
    ...
    subtitle = "Mean percentages of nine waste catergories displayed by income catergories",
    x = "Mean",
    y = "Waste Category",
